...
export function ChatMessage({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex max-w-[90%] md:max-w-[80%] gap-2 md:gap-3 items-end",
      ...
      >
        ...
        >
          ...
          {!isOwnMessage && showAvatar && (
            <span className="text-[12px] md:text-[13px] font-semibold text-zinc-300 mb-1 ml-1">
              {user.name}
            </span>
          )}
          ...
          >
            {type === "image" ? (
              viewed && mode === "view-once" ? (
                <span className="text-[14px] md:text-[15px] font-medium leading-relaxed">
                  Image expired
                </span>
              ) : (
                <button
                ...
                >
                  {mode === "persistent" && imageUrl ? (
                    <img
                      src={imageUrl}
                      alt="Shared image"
                      loading="lazy"
                      className="max-h-56 w-auto rounded-md border border-zinc-700"
                    />
                  ) : (
                    <div className="rounded-md border border-zinc-700 bg-zinc-900 px-3 py-4 text-sm">
                      Tap to view image
                    </div>
                  )}
                  <span className="text-xs opacity-80">
                    {mode === "view-once" ? "View Once" : "Keep Image"}
                  </span>
                </button>
              )
            ) : (
              <span className="text-[15px] md:text-base font-medium leading-relaxed break-words whitespace-pre-wrap">
                {message}
              </span>
            )}
            <span
              className={cn(
                ...
                isOwnMessage
                  ? "text-blue-100 self-end"
                  : "text-zinc-400 self-start",
              )}
            >
              {timeString}
            </span>
